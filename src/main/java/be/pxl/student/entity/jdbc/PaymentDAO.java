...
public class PaymentDAO implements DAO<Payment, PaymentException> {
    private static final String CREATE = "INSERT INTO Payment (`accountId`, `counterAccountId`, `date`, `amount`, `currency`, `detail`) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    private static final String UPDATE = "UPDATE Payment SET `accountId` = ?, `counterAccountId` = ?, `date` = ?, `amount` = ?, `currency` = ?, `detail` = ? WHERE id = ?";
    ...
    public Payment create(Payment payment) throws PaymentException {
        try(PreparedStatement preparedStatement =  manager.getConnection().prepareStatement(CREATE)) {
            ...
            preparedStatement.setInt(1, payment.getAccount().getId());
            preparedStatement.setInt(2, payment.getCounterAccount().getId());


            //preparedStatement.setString(3, payment.getIBAN());
            preparedStatement.setDate(3,  (Date) payment.getDate());
            preparedStatement.setFloat(4, payment.getAmount());
            preparedStatement.setString(5, payment.getCurrency());
            preparedStatement.setString(6, payment.getDetail());
